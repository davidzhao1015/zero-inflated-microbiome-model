...
zero_summary_innocuum <- abund_meta_rectum5 %>%
  ...
  summarise(sample_size = n(),
            ...
            average_read = round(mean(read), 2)) 
...
```
The `Log(theta)` value indicates that data are over-dispersed. 
...
Compare four models, `ZIP_innocuum`, `ZINB_innocuum`, `ZHP_innocuum`, and `ZHNB_innocuum` based on alternative statsitics (i.e., likelihood ratio, AIC, BIC and Vuong test). 

Use `lmtest` and `nonnest2` packages for statistics computation. 
...
# AIC
t(AIC(ZIP_innocuum, ZINB_innocuum, ZHP_innocuum, ZHNB_innocuum)) # lower is better 


# BIC 
nonnest2::icci(ZIP_innocuum, ZINB_innocuum, conf.level = 0.95)

nonnest2::icci(ZHP_innocuum, ZHNB_innocuum)


# Vuong test 
vuong(ZIP_innocuum, ZHP_innocuum)

vuong(ZIP_innocuum, ZHNB_innocuum)

vuong(ZINB_innocuum, ZHP_innocuum) 

vuong(ZINB_innocuum, ZHNB_innocuum) 

vuong(ZHNB_innocuum, ZHP_innocuum) 
