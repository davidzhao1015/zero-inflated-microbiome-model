...
The response variable is `read` (abundance) of the taxa `_Clostridium_innocuum_group`, and the independent (explanatory) variable is `diagnosis` which is a multi-level, categorical variable. The offset term is logarithm value of total read in individual samples. 
...
Compare four models, `ZIP_innocuum`, `ZINB_innocuum`, `ZHP_innocuum`, and `ZHNB_innocuum` based on alternative statsitics (i.e., likelihood ratio, and AIC). 
...
```
According to AIC,  `ZINB_innocuum` seems the best model in the present case, since it has the smallest AIC relative to other three counterparts. 
